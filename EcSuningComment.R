...
library(tmcn)
library(stringr)
setwd("/Volumes/Share/Data/外部數據-蘇寧/20170921SuningComment")
web_all = list.files()
...
for(i in 1:length(web_all)){
  ...
  #product
  product = web %>% html_nodes("#itemDisplayName") %>% html_text()
  product = str_trim(product, side = "both")
  product = iconv(product)
  product = toTrad(product, rev=T)
  
  #postdate
  postDate = web %>% html_nodes(".date span") %>% html_text()
  ...
  #comment
  comment = web %>% html_nodes(".body-content") %>% html_text()
  ...
  #starnumber
  starNumber = web %>% html_nodes(".rv-spring.on") %>% html_attr("style")
  starNumber = substr(starNumber, 8, 9)
  starNumber = as.numeric(starNumber)/14
  
  #likenumber
  likeNumber = web %>% html_nodes(".pjnr_youyong em")%>% html_text()
  likeNumber = gsub("\\(","",likeNumber)
  likeNumber = gsub("\\)","",likeNumber)
  likeNumber = as.integer(likeNumber)
  
  #url
  url = web %>% html_nodes("body > div.wrapper.proinfo > div.proinfo-container.clearfix > div.proinfo-left > div.imgzoom-memo > div.share > div > a.sina") %>% html_attr("href")
  url = substr(url, unlist(gregexpr("url", url))[1]+4, unlist(gregexpr(".html", url))[1]+4)
  if(length(url) == 0){
    url = web %>% html_nodes("link") %>% html_attr("href")
    url = paste0("https:", url[which(grepl("//product.suning.com/", url))[1]])
  }
  
  #productId
  productID = substr(url, unlist(gregexpr("[/]", url))[length(unlist(gregexpr("[/]", url)))]+1, unlist(gregexpr(".html", url))[1]-1)
  ...
  for(j in 1:length(comment)){
    ...
    if(is.na(postDate[1])){
      a = list("_id" = unbox(paste0("SUNING_", cday, str_pad(m$count(query = toJSON(list(crawlDate = unbox(str_pad(cday, 
                                          17, side = "right", pad = "0")))))+1, 6, side = "left", pad = "0"))),
               productID = unbox(productID),
               product = unbox(product),
               postDate = unbox(NA),
               commentID = unbox(paste0("SUNING_", cday, str_pad(m$count(query = toJSON(list(crawlDate = unbox(str_pad(cday, 
                                           17, side = "right", pad = "0")))))+1, 6, side = "left", pad = "0"))),
               commentFlavor = unbox(NA),
               comment = unbox(comment[j]),
               starNumber = unbox(as.integer(starNumber[j])),
               likeNumber = unbox(as.integer(likeNumber[j])),
...
for(i in 1:length(web_prod)){
  ...
  #goodRate
  goodRate = web %>% html_nodes(".score") %>% html_text() 
  goodRate = as.numeric(gsub("%", "", goodRate))*0.01
  
  
  #commentTagList
  commentTagList = web %>% html_nodes("#appraise .label") %>% html_text() 
  commentTagList = sapply(commentTagList, 
                          function(x) substr(x, 1, unlist(gregexpr("\\(", x))[1]-1))
  names(commentTagList) = NULL
  #commentNumber
  commentNumber = web %>% html_nodes(".now span") %>% html_text() 
  commentNumber = as.numeric(gsub("[\\(\\)]", "", commentNumber))[1]
...
m = rmongo()
#Product的資訊
crawlDate = unlist(lapply(m$iterate()$batch(1290), function(x) x$crawlDate))
crawlnum = c(which(grepl("20170920",crawlDate)),which(grepl("20170921",crawlDate)))
crawlDate = crawlDate[crawlnum]

url = unlist(lapply(m$iterate()$batch(1290), function(x) x$url))
url = url[crawlnum]

keywordList = lapply(m$iterate()$batch(1290), function(x) x$keywordList)
